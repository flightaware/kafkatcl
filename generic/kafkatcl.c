 ...
 */
int
kafkatcl_createTopicObjectCommand (kafkatcl_handleClientData *kh, char *commandName, const char *topic)
{
...
#define TOPIC_STRING_FORMAT "kafka_topic%lu"
	...
	if (strcmp (commandName, "#auto") == 0) {
		...
		snprintf (commandName, baseNameLength, TOPIC_STRING_FORMAT, nextAutoCounter++);
		autoGeneratedName = 1;
	}

	// create a Tcl command to interface to the topic object
	kt->cmdToken = Tcl_CreateObjCommand (interp, commandName, kafkatcl_topicObjectObjCmd, kt, kafkatcl_topicObjectDelete);
...
kafkatcl_handleObjectObjCmd(ClientData cData, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[])
{
    ...
    switch ((enum options) optIndex) {
		...
		case OPT_NEW_TOPIC: {
			if (objc != 4) {
				Tcl_WrongNumArgs (interp, 4, objv, "commandName topic");
				return TCL_ERROR;
			}
			...
			resultCode = kafkatcl_createTopicObjectCommand (kh, commandName, topic);
 ...
 */
int
kafkatcl_createHandleObjectCommand (kafkatcl_objectClientData *ko, char *commandName, rd_kafka_type_t kafkaType)
{
	char errStr[256];
...
#define HANDLE_STRING_FORMAT "kafka_handle%lu"
	...
	if (strcmp (commandName, "#auto") == 0) {
		...
		commandName = ckalloc (baseNameLength);
		snprintf (commandName, baseNameLength, HANDLE_STRING_FORMAT, nextAutoCounter++);
		autoGeneratedName = 1;
	}

	// create a Tcl command to interface to the handle object
	kh->cmdToken = Tcl_CreateObjCommand (interp, commandName, kafkatcl_handleObjectObjCmd, kh, kafkatcl_handleObjectDelete);
	// set the full name to the command in the interpreter result
...
kafkatcl_kafkaObjCmd(ClientData clientData, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[])
{
    ...
    switch ((enum options) optIndex) {
		case OPT_CREATE: {
			...
			ko->cmdToken = Tcl_CreateObjCommand (interp, commandName, kafkatcl_kafkaObjectObjCmd, ko, kafkatcl_kafkaObjectDelete);
